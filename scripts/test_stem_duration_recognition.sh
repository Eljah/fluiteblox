...
OUT_DIR="$ROOT/target/stem-duration-test"
rm -rf "$OUT_DIR"
...
public class Bitmap {
    ...
    public static Bitmap createBitmap(int width, int height, Config ignored) {
...
public class AssetManager {
    public InputStream open(String name) { return null; }
}
JAVA
...
javac -cp "$OPENCV_JAR" -d "$OUT_DIR" \
  ...
  "$ROOT/src/test/java/tatar/eljah/recorder/StemDurationRecognitionRegressionTest.java"

java -cp "$OUT_DIR:$OPENCV_JAR" tatar.eljah.recorder.StemDurationRecognitionRegressionTest
