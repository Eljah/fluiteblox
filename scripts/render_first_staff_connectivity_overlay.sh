...
OUT_DIR="$ROOT/target/first-staff-overlay"
rm -rf "$OUT_DIR"
...
javac -cp "$OPENCV_JAR" -d "$OUT_DIR" \
  ...
  "$ROOT/src/test/java/tatar/eljah/recorder/FirstStaffConnectivityOverlayRenderer.java"

INPUT="${1:-$ROOT/clear_sreenshot.png}"
OUTPUT="${2:-$ROOT/target/first_staff_connectivity_overlay.png}"
java -cp "$OUT_DIR:$OPENCV_JAR" tatar.eljah.recorder.FirstStaffConnectivityOverlayRenderer "$INPUT" "$OUTPUT"
